...
async function run() {
  ...
  if (!order) {
    console.log('Order 1170 not found');
    return;
  }
  const shopifyId = order.shopify_order_id;
  const { data, error } = await supabase.from('shopify_order_events').select('topic, event_data, raw_payload').eq('shopify_order_id', shopifyId).order('created_at', { ascending: true });
